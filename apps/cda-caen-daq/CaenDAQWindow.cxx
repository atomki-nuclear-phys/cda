...
CaenDAQWindow::CaenDAQWindow(const QString& confFileName, msg::Level verbosity)
   ...
   if (!m_msgServer->listen(Address(MSG_SERVER_ADDRESS))) {
      QMessageBox::critical(
          this, tr("Error starting message server"),
   ...
   //
   m_eventReceiversLabel =
       new QLabel(tr("Extra event receivers:"), m_centralWidget);
   ...
   //
   m_statReceiversLabel =
       new QLabel(tr("Extra statistics receivers:"), m_centralWidget);
   ...
   if (!confFileName.isEmpty()) {
      ...
      m_logger << msg::INFO
               << tr("Please open a configuration file before"
...
/**
 * The destructor actually does something in this class, it has to delete quite
 * a few objects which were created in the constructor.
 */
...
void CaenDAQWindow::drawMenus() {
   ...
   QAction* aboutCaenDaqAc =
       helpMenu->addAction(QIcon(":/img/caen.png"), tr("About Caen DAQ"));
   connect(aboutCaenDaqAc, SIGNAL(triggered()), this, SLOT(aboutCaenDAQSlot()));
