...
namespace raw {
   ...
   bool Crate::initialize( const QString& fileName ) {
      ...
      if( m_file ) {
         m_logger << msg::ERROR
                  << tr( "Output file is already open" )
                  << msg::endmsg;
         return false;
      }
      ...
      if( ! m_file->open( QFile::WriteOnly | QFile::Truncate ) ) {
         m_logger << msg::ERROR
                  << tr( "%1 couldn't be opened for writing" ).arg( fileName )
                  << msg::endmsg;
         return false;
      } else {
         m_logger << msg::DEBUG
      ...
      if( ! writeConfig( m_file ) ) {
         m_logger << msg::ERROR
                  << tr( "Couldn't write the configuration into the output file" )
                  << msg::endmsg;
         return false;
      } else {
         m_logger << msg::DEBUG
   ...
   bool Crate::writeEvent( const ev::Event& event ) {
      ...
      if( ( ! m_stream ) || ( ! m_file ) ) {
         m_logger << msg::ERROR
                  << tr( "Trying to write an event before opening an output file" )
                  << msg::endmsg;
         return false;
      }
