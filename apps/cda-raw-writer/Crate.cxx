...
bool Crate::initialize(const QString& fileName) {
   ...
   if (!m_file->open(QFile::WriteOnly | QFile::Truncate)) {
      REPORT_ERROR(tr("%1 couldn't be opened for writing").arg(fileName));
      return false;
   } else {
   ...
   if (!writeConfig(*m_file)) {
      REPORT_ERROR(
          tr("Couldn't write the configuration into the "
             "output file"));
      return false;
   } else {
      m_logger << msg::DEBUG << tr("Configuration written into the output file")
               << msg::endmsg;
   }
   ...
   if (m_stream) {
      REPORT_ERROR(
          tr("The object is in a very weird state. No file "
   ...
   m_logger << msg::INFO << tr("%1 opened for writing").arg(fileName)
            << msg::endmsg;
...
bool Crate::writeEvent(const ev::Event& event) {
   ...
   if ((!m_stream) || (!m_file)) {
      REPORT_ERROR(
          tr("Trying to write an event before opening an "
             "output file"));
      return false;
   }
   ...
   *m_stream << event;
   ...
   if (m_evCounter >= 100) {
      if (!m_file->flush()) {
         REPORT_ERROR(
             tr("There was an error flushing the "
                "output file"));
         return false;
      }
      m_evCounter = 0;
