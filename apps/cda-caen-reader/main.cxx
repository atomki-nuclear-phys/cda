...
int main(int argc, char* argv[]) {
   ...
   cmd.add(config);
   TCLAP::MultiArg<std::string> clients("e", "clients",
   ...
   //
   g_logger << msg::INFO
            << qApp->translate("cda-caen-reader", "CAEN readout running...")
            << msg::endmsg;
...
void shutDown(int) {
   ...
   g_logger << msg::DEBUG
            << qApp->translate("cda-caen-reader", "Starting shutdown procedure")
            << msg::endmsg;
   ...
   if (!g_crate->stop().isSuccess()) {
      ...
      g_logger << msg::DEBUG
               << qApp->translate("cda-caen-reader", "Data acquisition stopped")
               << msg::endmsg;
   }
   ...
   g_logger << msg::INFO
            ...
            << msg::endmsg;
   g_logger << msg::INFO
            << qApp->translate("cda-caen-reader", "Terminating application...")
            << msg::endmsg;
   exit(0);
