...
CernlibDisk::CernlibDisk() : m_processor(), m_logger("dt5740::CernlibDisk") {
...
bool CernlibDisk::initialize(cernlib::NTupleMgr& nmgr) {
   ...
   for (int group = 0; group < NUMBER_OF_GROUPS; ++group) {
      for (int channel = 0; channel < GroupConfig::CHANNELS_IN_GROUP;
         ...
         m_ntupleMap[group][channel][0] = nmgr.addVar(ch->getTimeName());
         m_ntupleMap[group][channel][1] = nmgr.addVar(ch->getEnergyName());
      }
   }
