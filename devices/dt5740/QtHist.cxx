...
QtHist::QtHist(QWidget* parent, Qt::WindowFlags flags)
   ...
   m_channelTab->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
...
bool QtHist::initialize() {
   ...
   for (int group = 0; group < NUMBER_OF_GROUPS; ++group) {
      for (int channel = 0; channel < GroupConfig::CHANNELS_IN_GROUP;
         ...
         m_rawHistograms[group][channel].reset(new RawHistogram(
             ch->getRawName(), getSamples(), -0.5, getSamples() - 0.5, 5000));
         ...
         m_histograms[group][channel][0].reset(new moni::Histogram(
             ch->getTimeName(), ch->getTimeNumberOfChannels(),
             ch->getTimeLowerBound(), ch->getTimeUpperBound()));
         m_histograms[group][channel][0]->setHistColor(QColor(Qt::darkBlue));
         m_layouts[group][channel]->addWidget(
             m_histograms[group][channel][0].get());

         m_histograms[group][channel][1].reset(new moni::Histogram(
             ch->getEnergyName(), ch->getEnergyNumberOfChannels(),
             ch->getEnergyLowerBound(), ch->getEnergyUpperBound()));
