...
RootDisk::RootDisk() : m_processor(), m_logger("dt5740::RootDisk") {}
...
bool RootDisk::initialize(root::NTupleMgr& nmgr) {
   ...
   for (int group = 0; group < NUMBER_OF_GROUPS; ++group) {
      for (int channel = 0; channel < GroupConfig::CHANNELS_IN_GROUP;
         ...
         CHECK(nmgr.addVar(
             m_times[group][channel], ch->getTimeName(),
             tr("Time of channel %1 of group %2 in a %3 with device "
                "ID %4")
                 ...
                 .arg(getID())));
         CHECK(nmgr.addVar(
             m_energies[group][channel], ch->getEnergyName(),
         ...
         if (m_saveRawNtuple) {
            ...
            CHECK(nmgr.addVar(
                m_channelData[group][channel], ch->getRawName(),
