...
RawHistogram::RawHistogram(const QString& title, int bins, double low,
                           double up, int refreshTimeout, QWidget* parent,
                           Qt::WindowFlags flags)
    : moni::Histogram(title, bins, low, up, refreshTimeout, parent, flags),
      Processor(),
      m_time(0.0),
      m_energy(0.0),
      m_transScale(1.0) {
...
void RawHistogram::paintEvent(QPaintEvent* event) {
   ...
   if (m_smooth.size()) {
      ...
      for (size_t i = 0; i < m_smooth.size() - 1; ++i) {
         ...
         const int y_bin_pos = static_cast<int>(
             std::floor(y_axis_start - ybin.getDrawPosition(m_smooth[i])));
   ...
   if (m_trans.size()) {
      ...
      for (size_t i = 0; i < m_trans.size() - 1; ++i) {
         ...
         const int y_bin_pos = static_cast<int>(std::floor(
             y_axis_start - ybin.getDrawPosition(m_transScale * m_trans[i])));
         const int x_bin_low_pos = static_cast<int>(
             std::floor(xbin.getDrawPosition(m_low + i * bin_width)));
         const int x_bin_up_pos = static_cast<int>(
             std::floor(xbin.getDrawPosition(m_low + (i + 1) * bin_width)));
   ...
   if (ybin.getDrawPosition(m_energy) > 0.0) {
      const int energy_pos = static_cast<int>(
          std::floor(y_axis_start - ybin.getDrawPosition(m_energy)));
      painter.setPen(QPen(QBrush(QColor(Qt::darkGreen)), 2));
      painter.drawLine(
          QLine(x_axis_start, energy_pos, width() - 20, energy_pos));
   }

   // Draw the reconstructed time:
   if (xbin.getDrawPosition(m_time) > 0.0) {
      const int time_pos = static_cast<int>(
          std::floor(x_axis_start + xbin.getDrawPosition(m_time)));
      painter.setPen(QPen(QBrush(QColor(Qt::darkBlue)), 2));
      painter.drawLine(QLine(time_pos, 20, time_pos, y_axis_start));
   }
...
/**
 * This function is re-implemented in order to take the limits of the
 * transformed raw distribution into account.
 *
 ...
 * goes outside of the limits of the raw distribution, this re-implemented
 * function constructs the limits such that both distributions would fit inside
 * of it.
 *
 ...
 */
std::pair<double, double> RawHistogram::getYAxisLimits() const {
   ...
   const double tmaximum =
       (m_trans.size() ? *(std::max_element((m_trans.begin() + tstart),
                                            (m_trans.begin() + tend)))
                       : 0.0);
   m_transScale = (std::abs(tmaximum) > 0.001 ? 0.8 * maximum / tmaximum : 1.0);

   double view_maximum = 0.0;

   // Get the minimum value within the limits:
   const double minimum =
       (m_trans.size() ? std::min(*(std::min_element((m_values.begin() + start),
                                                     (m_values.begin() + end))),
                                  m_transScale * (*std::min_element(
                                                     (m_trans.begin() + tstart),
                                                     (m_trans.begin() + tend))))
...
/**
 * In principle I don't see a scenario when looking at this distribution in
 * log-scale would make too much sense.
 */
...
void RawHistogram::configureToolTip() {
   ...
   setToolTip(
       tr("The lines/histograms mean the following:\n"
