 ...
 */
bool Processor::reconstruct(const std::vector<uint16_t>& data, Result& result) {
   ...
   if (std::abs(m_smoothWidth) > 0.001) {
      for (size_t i = 0; i < data.size(); ++i) {
         ...
         for (size_t j = 0; j < data.size(); ++j) {
            ...
            const int dist = (static_cast<int>(j) - static_cast<int>(i));
            // Add the contribution of this point to the one being smoothed:
            m_smooth[i] += (gauss(dist) * static_cast<double>(data[j]));
         }
      }
   } else {
      for (size_t i = 0; i < data.size(); ++i) {
   ...
   for (size_t k = 0; k < m_smooth.size(); ++k) {
      ...
      if ((k >= 1) && (k < (m_smooth.size() - m_delay))) {
         for (int i = 0; i < m_length; ++i) {
            m_trans[k] +=
                (m_smooth[k - 1] - m_fraction * m_smooth[k - i + m_delay]);
         }
      }
   ...
   for (size_t i = 0; i < m_trans.size() - 4; ++i) {
      if ((m_trans[i] < 0.0) && (m_trans[i + 1] < 0.0) &&
          (m_trans[i + 2] > 0.0) && (m_trans[i + 3] > 0.0)) {
         result.time = (double)i +
                       3.0 * (std::abs(m_trans[i]) / std::abs(m_trans[i + 4]));
         break;
      }
   }
...
double Processor::gauss(int x) const {
   ...
   if (std::abs(static_cast<double>(x)) > 5 * std::abs(m_smoothWidth)) {
      return 0.0;
   }
