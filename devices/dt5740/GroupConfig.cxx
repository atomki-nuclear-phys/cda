...
GroupConfig::GroupConfig(const GroupConfig& parent)
   ...
   for (int i = 0; i < CHANNELS_IN_GROUP; ++i) {
      if (parent.m_channels[i]) {
         m_channels[i].reset(new ChannelConfig(*(parent.m_channels[i])));
      } else {
         m_channels[i].reset();
      }
   }
...
GroupConfig& GroupConfig::operator=(const GroupConfig& rh) {
   ...
   for (int i = 0; i < CHANNELS_IN_GROUP; ++i) {
      if (rh.m_channels[i]) {
         m_channels[i].reset(new ChannelConfig(*(rh.m_channels[i])));
      } else {
         m_channels[i].reset();
      }
   }
...
StatusCode GroupConfig::readConfig(const QDomElement& node) {
   ...
   m_trigOutEnabled = node.attribute("TrigOutEnabled", "0").toInt(&ok);
   CHECK(ok);
...
StatusCode GroupConfig::writeConfig(QDomElement& node) const {
   ...
   for (int i = 0; i < CHANNELS_IN_GROUP; ++i) {
      if (m_channels[i]) {
         QDomElement ch_element = node.ownerDocument().createElement("Channel");
...
ChannelConfig* GroupConfig::getChannel(int channel) {
   ...
   if ((channel >= 0) && (channel < CHANNELS_IN_GROUP)) {
