...
StatusCode Readout::initialize() {
   ...
   // Some basic settings:
   REPORT_VERBOSE(tr("Record length will be: %1").arg(getSamples()));
   CHECK(m_digitizer.setRecordLength(getSamples()));
   REPORT_VERBOSE(tr("Group enable mask will be: %1").arg(groupMask()));
   ...
   for (int i = 0; i < NUMBER_OF_GROUPS; ++i) {
      REPORT_VERBOSE(tr("Setting trigger threshold for group %1 to %2")
                         .arg(i)
                         .arg(m_groups[i].getTrigThreshold()));
      CHECK(m_digitizer.setGroupTriggerThreshold(
          i, m_groups[i].getTrigThreshold()));
      REPORT_VERBOSE(tr("Setting self trigger mode for group %1 to %2")
                         .arg(i)
                         .arg(trigMode(m_groups[i])));
      CHECK(m_digitizer.setGroupSelfTriggerMode(i, trigMode(m_groups[i])));
      REPORT_VERBOSE(tr("Setting channel mask of group %1 to %2")
                         .arg(i)
                         .arg(m_groups[i].getTrigMask()));
      CHECK(m_digitizer.setChannelGroupMask(i, m_groups[i].getTrigMask()));
      REPORT_VERBOSE(tr("Setting DC offset of group %1 to %2")
                         .arg(i)
                         .arg(m_groups[i].getDCOffset()));
   ...
   REPORT_VERBOSE(tr("Setting external trigger mode to %1").arg(extTrigMode()));
   ...
   CHECK(m_digitizer.writeRegister(REG_GROUP_CONFIG, groupConfReg()));
   CHECK(m_digitizer.writeRegister(REG_ACQ_CONTROL, acqControlReg()));
   CHECK(m_digitizer.writeRegister(REG_IO_CONTROL, ioControlReg()));
...
std::unique_ptr<ev::Fragment> Readout::readEvent() {
   ...
   if (!m_numEvents) {
      ...
      for (;;) {
         ...
         if (!m_digitizer.getNumEvents(m_buffer, m_eventSize, m_numEvents)) {
            REPORT_ERROR(
                tr("Couldn't get the number of events for the "
        ...
        // If there are events available, exit the loop:
         ...
         if (g_stopAcquisition) {
            REPORT_VERBOSE(
                tr("The acquisition is to be stopped. Returning "
   ...
   if (!m_digitizer.getEvent(m_buffer, m_bufferSize, m_currentEvent,
                             m_eventInfo, m_event)) {
