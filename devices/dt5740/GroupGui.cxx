...
GroupGui::GroupGui(GroupConfig& group, QWidget* parent, Qt::WindowFlags flags)
    : QWidget(parent, flags), m_group(group), m_syncing(false) {
   ...
   //
   m_groupBox.reset(new QGroupBox(
       tr("Channel group %1").arg(m_group.getGroupNumber()), this));
   ...
   //
   m_trigEnabled.reset(
       new QCheckBox(tr("Enable self trigger"), m_groupBox.get()));
   m_trigEnabled->setGeometry(QRect(10, 25, 400, 25));
   connect(m_trigEnabled.get(), SIGNAL(toggled(bool)), this,
           SLOT(trigEnabledSlot(bool)));

   m_trigOutEnabled.reset(new QCheckBox(tr("Forward self trigger to "
                                           "front panel"),
                                        m_groupBox.get()));
   m_trigOutEnabled->setGeometry(QRect(10, 55, 400, 25));
   connect(m_trigOutEnabled.get(), SIGNAL(toggled(bool)), this,
           SLOT(trigOutEnabledSlot(bool)));

   //
   // Create the trigger threshold selector:
   //
   m_trigThresholdLabel.reset(
       new QLabel(tr("Trigger threshold:"), m_groupBox.get()));
   ...
   connect(m_trigThreshold.get(), SIGNAL(valueChanged(int)), this,
           SLOT(trigThresholdSlot(int)));

   //
   // Create the DC offset setting:
   //
   m_dcOffsetLabel.reset(new QLabel(tr("DC offset:"), m_groupBox.get()));
   ...
   connect(m_dcOffset.get(), SIGNAL(valueChanged(int)), this,
           SLOT(dcOffsetSlot(int)));
   ...
   m_trigMaskBox->setToolTip(
       "You can set here which channels should "
   ...
   for (int i = 0; i < GroupConfig::CHANNELS_IN_GROUP; ++i) {
      m_trigMask[i].reset(new QCheckBox(
          tr("%1.").arg(
              m_group.getGroupNumber() * GroupConfig::CHANNELS_IN_GROUP + i),
          m_trigMaskBox.get()));
      m_trigMask[i]->setGeometry(QRect(5 + i * 55, 25, 50, 25));
      connect(m_trigMask[i].get(), SIGNAL(toggled(bool)), this,
              SLOT(trigMaskSlot(bool)));
   }
   ...
   for (int i = 0; i < GroupConfig::CHANNELS_IN_GROUP; ++i) {
      ...
      m_channels[i].reset(new ChannelGui(
          m_group.getGroupNumber() * GroupConfig::CHANNELS_IN_GROUP + i,
          m_groupBox.get()));
      m_channels[i]->setGeometry(
          QRect(10, 240 + i * 75, ChannelGui::WIDTH, ChannelGui::HEIGHT));

      connect(m_channels[i].get(), SIGNAL(enableChanged(int, bool)), this,
              SLOT(channelEnabledSlot(int, bool)));
      connect(m_channels[i].get(), SIGNAL(rawNameChanged(int, const QString&)),
              this, SLOT(rawNameChangedSlot(int, const QString&)));

      connect(m_channels[i].get(), SIGNAL(timeNameChanged(int, const QString&)),
              this, SLOT(timeNameChangedSlot(int, const QString&)));
      connect(m_channels[i].get(), SIGNAL(timeChannelsChanged(int, int)), this,
              SLOT(timeChannelsChangedSlot(int, int)));
      connect(m_channels[i].get(), SIGNAL(timeLowerBoundChanged(int, double)),
              this, SLOT(timeLowerBoundChangedSlot(int, double)));
      connect(m_channels[i].get(), SIGNAL(timeUpperBoundChanged(int, double)),
              this, SLOT(timeUpperBoundChangedSlot(int, double)));

      connect(m_channels[i].get(),
              SIGNAL(energyNameChanged(int, const QString&)), this,
              SLOT(energyNameChangedSlot(int, const QString&)));
      connect(m_channels[i].get(), SIGNAL(energyChannelsChanged(int, int)),
              this, SLOT(energyChannelsChangedSlot(int, int)));
      connect(m_channels[i].get(), SIGNAL(energyLowerBoundChanged(int, double)),
              this, SLOT(energyLowerBoundChangedSlot(int, double)));
      connect(m_channels[i].get(), SIGNAL(energyUpperBoundChanged(int, double)),
...
ChannelGui* GroupGui::getChannel(int index) const {
   ...
   if ((index >= 0) && (index < GroupConfig::CHANNELS_IN_GROUP)) {
...
void GroupGui::channelEnabledSlot(int channel, bool on) {
   ...
   m_group.enableChannel(
       channel - (m_group.getGroupNumber() * GroupConfig::CHANNELS_IN_GROUP),
       on);
...
void GroupGui::rawNameChangedSlot(int channel, const QString& text) {
   ...
   ChannelConfig* c = m_group.getChannel(
       channel - (m_group.getGroupNumber() * GroupConfig::CHANNELS_IN_GROUP));
   c->setRawName(text);
   return;
}
