...
QtHist::QtHist(QWidget* parent, Qt::WindowFlags flags)
    : dev::QtHist(parent, flags), m_logger("v862::QtHist") {
...
QString QtHist::windowTitle() const {
   ...
   return (tr("V862 on address %1").arg(m_vmeAddress, 4, 16, QChar('0')));
}
...
bool QtHist::initialize() {
   ...
   for (size_t i = 0; i < NUMBER_OF_CHANNELS; ++i) {
      ...
      m_histograms[i].reset(new moni::Histogram(
          m_channels[i]->getName(), m_channels[i]->getNBins(),
          m_channels[i]->getLowerBound(), m_channels[i]->getUpperBound()));

      // Add it as a tab:
      m_channelTab->addTab(
          m_histograms[i].get(),
          tr("Channel %1 (%2)").arg(i + 1).arg(m_channels[i]->getName()));
   }
