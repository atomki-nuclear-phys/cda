...
StatusCode Device::readConfig(const QDomElement& element) {
   ...
   CHECK(ok);
   m_outputWidth1 = element.attribute("OutputWidth_0_7", "0").toInt(&ok);
   CHECK(ok);
   m_outputWidth2 = element.attribute("OutputWidth_8_15", "0").toInt(&ok);
   CHECK(ok);
   m_deadTime1 = element.attribute("DeadTime_0_7", "0").toInt(&ok);
   CHECK(ok);
   m_deadTime2 = element.attribute("DeadTime_8_15", "0").toInt(&ok);
   CHECK(ok);
   m_majorityThreshold = element.attribute("MajorityThreshold", "1").toInt(&ok);
   CHECK(ok);
