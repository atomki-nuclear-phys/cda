...
Readout::Readout() : Device(), m_logger("v775::Readout") {}

StatusCode Readout::initialize(const caen::VmeBus& bus) {

   // Open the connection to the hardware device:
   CHECK(m_vmeDevice.open(m_vmeAddress, caen::VmeDevice::DEV_V775A, bus));
   ...
   CHECK(m_vmeDevice.setAcquisitionMode(
       SLIDING_SCALE_ENABLE, m_zeroSuppressionEnabled,
       m_overflowSuppressionEnabled, m_validSuppressionEnabled,
       m_commonStopEnabled, EMPTY_ENABLE, COUNT_ALL_EVENTS));
   ...
   CHECK(m_vmeDevice.setReadoutMode(BUS_ERROR_ENABLE, BLOCK_END_ENABLE,
                                    ALIGN_64_ENABLE));
