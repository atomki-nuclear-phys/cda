...
StatusCode Device::readConfig(const QDomElement& element) {
   ...
   m_zeroSuppressionEnabled =
       element.attribute("ZeroSuppression", "0").toInt(&ok);
   CHECK(ok);

   // Read the overflow suppression flag:
   m_overflowSuppressionEnabled =
       element.attribute("OverflowSuppression", "0").toInt(&ok);
   CHECK(ok);

   // Read the zero suppression flag:
   m_validSuppressionEnabled =
       element.attribute("ValidSuppression", "0").toInt(&ok);
   CHECK(ok);

   // Read the common stop flag:
   m_commonStopEnabled = element.attribute("CommonStop", "0").toInt(&ok);
   CHECK(ok);

   // Read the Full Scale Range register value:
   m_fullScaleRangeValue =
       element.attribute("FullScaleRange", "255").toInt(&ok);
   CHECK(ok);
...
StatusCode Device::writeConfig(QDomElement& element) const {
   ...
   element.setAttribute("OverflowSuppression", m_overflowSuppressionEnabled);
...
Device::ChannelData::ChannelData(const caen::VmeDevice::DataWord& dw)
   ...
   m_data =
       ((dw.data & 0xffff) | ((dw.channel << 16) & 0xff0000) |
        (dw.underThreshold ? 0x1000000 : 0) | (dw.overflow ? 0x2000000 : 0));
}
