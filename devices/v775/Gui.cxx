...
Gui::Gui(QWidget* parent, Qt::WindowFlags flags)
    : dev::CaenVmeGui(parent, flags), m_logger("v775::Gui") {
   ...
   //
   m_deviceSettingsBox.reset(
       new QGroupBox(tr("Device properties"), m_scrollWidget.get()));
   m_deviceSettingsBox->setGeometry(QRect(130, 80, WIDGET_WIDTH - 150, 260));

   m_vmeAddressLabel.reset(
       new QLabel(tr("VME Address (hex)"), m_deviceSettingsBox.get()));
   ...
   connect(m_vmeAddressEdit.get(), SIGNAL(textChanged(QString)), this,
           SLOT(vmeAddressSlot(QString)));

   m_zeroSuppressionEdit.reset(new QCheckBox(tr("Zero suppression enabled"),
                                             m_deviceSettingsBox.get()));
   m_zeroSuppressionEdit->setGeometry(QRect(10, 60, 300, 25));
   connect(m_zeroSuppressionEdit.get(), SIGNAL(toggled(bool)), this,
           SLOT(zeroSuppressionSlot(bool)));

   m_overflowSuppressionEdit.reset(new QCheckBox(
       tr("Overflow suppression enabled"), m_deviceSettingsBox.get()));
   m_overflowSuppressionEdit->setGeometry(QRect(10, 85, 300, 25));
   connect(m_overflowSuppressionEdit.get(), SIGNAL(toggled(bool)), this,
           SLOT(overflowSuppressionSlot(bool)));

   m_validSuppressionEdit.reset(new QCheckBox(tr("Valid suppression enabled"),
                                              m_deviceSettingsBox.get()));
   m_validSuppressionEdit->setGeometry(QRect(10, 110, 300, 25));
   connect(m_validSuppressionEdit.get(), SIGNAL(toggled(bool)), this,
           SLOT(validSuppressionSlot(bool)));
   ...
   connect(m_commonStopEdit.get(), SIGNAL(currentIndexChanged(int)), this,
           SLOT(commonStopSlot(int)));

   m_fullScaleRangeBox.reset(
       new QGroupBox(tr("Full Scale Range"), m_deviceSettingsBox.get()));
   m_fullScaleRangeBox->setGeometry(QRect(10, 165, WIDGET_WIDTH - 170, 85));

   m_fullScaleRangeLabel.reset(
       new QLabel(tr("Register value"), m_fullScaleRangeBox.get()));
...
#endif
   connect(m_fullScaleRangeEdit.get(), SIGNAL(valueChanged(int)), this,
           SLOT(fullScaleRangeSlot(int)));

   const double stepValue = 8900.0 / m_fullScaleRangeValue;
   const double fullRange = stepValue * 4.096;
   m_fullScaleRangeRealValue.reset(new QLabel(
       tr(FSR_LABEL_VALUE).arg(fullRange, 0, 'f', 0).arg(stepValue, 0, 'f', 0),
   ...
   //
   m_channelSettingsBox.reset(
       new QGroupBox(tr("Input channels"), m_scrollWidget.get()));
   m_channelSettingsBox->setGeometry(QRect(10, 350, WIDGET_WIDTH - 30, 1650));
   m_channelNameLabel.reset(new QLabel(tr("Name"), m_channelSettingsBox.get()));
   m_channelNameLabel->setGeometry(QRect(135, 25, 75, 25));
   m_channelNameLabel->setAlignment(Qt::AlignCenter);
   m_channelNBinsLabel.reset(
       new QLabel(tr("NBins"), m_channelSettingsBox.get()));
   ...
   for (size_t i = 0; i < NUMBER_OF_CHANNELS; ++i) {
      m_gchannels[i].reset(
          new ChannelGui(static_cast<int>(i), m_channelSettingsBox.get()));
      m_gchannels[i]->setGeometry(
          QRect(20, static_cast<int>(50 + i * ChannelGui::HEIGHT),
                ChannelGui::WIDTH - 40, ChannelGui::HEIGHT));
      connect(m_gchannels[i].get(), SIGNAL(enableChanged(int, bool)), this,
              SLOT(channelEnabledSlot(int, bool)));
      connect(m_gchannels[i].get(), SIGNAL(nameChanged(int, QString)), this,
              SLOT(nameChangedSlot(int, QString)));
      connect(m_gchannels[i].get(), SIGNAL(nbinsChanged(int, int)), this,
              SLOT(nbinsChangedSlot(int, int)));
      ...
      connect(m_gchannels[i].get(), SIGNAL(thresholdChanged(int, int)), this,
              SLOT(thresholdChangedSlot(int, int)));
   }
}
...
void Gui::vmeAddressSlot(const QString& value) {
   ...
   if (!ok) {
      REPORT_ERROR(tr("Couldn't parse \"%1\" into hex value").arg(value));
   } else {
      emit addressChanged();
   }
...
void Gui::fullScaleRangeSlot(int value) {
   ...
   const double stepValue = 8900.0 / m_fullScaleRangeValue;
   const double fullRange = stepValue * 4.096;
   m_fullScaleRangeRealValue->setText(
       tr(FSR_LABEL_VALUE).arg(fullRange, 0, 'f', 0).arg(stepValue, 0, 'f', 0));
   return;
}
...
void Gui::sync() {
   ...
   m_vmeAddressEdit->setText(
       QString("%1").arg(m_vmeAddress, 4, 16, QChar('0')));
