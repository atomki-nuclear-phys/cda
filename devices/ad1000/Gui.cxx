...
Gui::Gui(QWidget* parent, Qt::WindowFlags flags)
   ...
   //
   m_topLabel.reset(new QLabel(tr("AD1000 Analog-to-Digital Converter"), this));
   ...
   m_gchannel->setGeometry(
       QRect(35, 100, ChannelGui::WIDTH, ChannelGui::HEIGHT));
   ...
   connect(m_gchannel.get(), SIGNAL(nameChanged(const QString&)), this,
           SLOT(nameChangedSlot(const QString&)));
   connect(m_gchannel.get(), SIGNAL(channelsChanged(int)), this,
           SLOT(channelsChangedSlot(int)));
   connect(m_gchannel.get(), SIGNAL(lowerBoundChanged(double)), this,
           SLOT(lowerBoundChangedSlot(double)));
   connect(m_gchannel.get(), SIGNAL(upperBoundChanged(double)), this,
...
void Gui::drawModule(QPainter& painter) const {
   ...
   //
   // Unfortunately I don't know what the module looks like, so let's just use
