...
bool NTupleMgr::openFile(const QString& fileName, const QString& treeName) {
   ...
   if (!m_file) {
      REPORT_ERROR(
          tr("Failed creating output file with name: %1").arg(fileName));
      return false;
   } else {
      m_logger << msg::INFO << tr("\"%1\" opened").arg(fileName) << msg::endmsg;
   }
   m_file->cd();

   // Create the output TTree:
   m_tree =
       new ::TTree(treeName.toLatin1().constData(), "Data collected by CDA");
...
bool NTupleMgr::closeFile() {
   ...
   if (!m_fileOpen) {
      m_logger << msg::DEBUG << tr("No output file is open at the moment")
               << msg::endmsg;
      return true;
   }
   ...
   m_logger << msg::INFO
            << tr("Closed output file after writing %1 events").arg(events)
            << msg::endmsg;
...
const char* NTupleMgr::rootType(const char* typeid_type) const {
   ...
   switch (typeid_type[0]) {
      ...
      default:
         REPORT_ERROR(
             tr("Unknown primitive type encountered: %1").arg(typeid_type));
         return "";
         break;
   }

   REPORT_ERROR(tr("Unknown primitive type encountered: %1").arg(typeid_type));
   return "";
}
...
bool NTupleMgr::checkBranch(const char* name) const {
   ...
   if ((!m_fileOpen) || (!m_tree)) {
      REPORT_ERROR(
          tr("You have to open an output file before declaring "
             "variables"));
      return false;
   }
   ...
   if (m_tree->GetBranch(name)) {
      REPORT_ERROR(tr("A branch with name \"%1\" already exists").arg(name));
      return false;
   }
#else
   REPORT_VERBOSE(
       tr("Checking if branch with name \"%1\" can be added").arg(name));
#endif  // HAVE_ROOT_LIBS

   return true;
}

bool NTupleMgr::makeBranch(const char* name,
                           void*
#ifdef HAVE_ROOT_LIBS
                               address
#endif  // HAVE_ROOT_LIBS
   ...
   if (!br) {
      REPORT_ERROR(tr("Failed creating branch with name: %1").arg(name));
      return false;
   }
   // Set its title:
   br->SetTitle(title);
