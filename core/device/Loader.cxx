...
bool Loader::loadAll() {
   ...
   if (!loadStaticPlugins()) {
      ...
      m_logger << msg::DEBUG << tr("Loaded the static plugins") << msg::endmsg;
   }
   ...
   for (QStringList::const_iterator pname = files.begin(); pname != files.end();
      ...
      if (QLibrary::isLibrary(*pname)) {
         ...
         if (this->load(*pname)) {
            ...
            m_logger << msg::WARNING << tr("%1 could not be loaded").arg(*pname)
                     << msg::endmsg;
         }
      }
   }
...
bool Loader::loadStaticPlugins() {
   ...
   for (QObjectList::iterator plugin = plugin_list.begin();
      ...
      if (!factory) {
         REPORT_ERROR(
             tr("Found a static plugin not providing "
                "the needed interface"));
         return false;
      } else {
         REPORT_VERBOSE(
             tr("Accessed dev::Factory interface in "
                "static plugin"));
      }
...
QStringList Loader::getDeviceNames() const {
   ...
   for (std::map<QString, Factory*>::const_iterator it = m_deviceMap.begin();
        it != m_deviceMap.end(); ++it) {
...
Factory& Loader::getFactory(const QString& name) const {
   ...
   std::map<QString, Factory*>::const_iterator it = m_deviceMap.find(name);
 ...
 * @param path Name of the directory holding the plugins
 */
Loader::Loader(const QString& path) : m_path(path), m_logger("dev::Loader") {
   ...
   if (m_path.isEmpty()) {
      ...
      m_logger << msg::DEBUG
               << tr("Setting device plugin directory "
                     "to: %1")
                      .arg(m_path)
               << msg::endmsg;
   }
