 ...
 */
HistMgr::HistMgr()
    : m_initialized(false), m_counter(0), m_logger("cernlib::HistMgr") {
...
void HistMgr::initialize(const char* gname) {
...
#ifndef HAVE_CERNLIB
   m_logger << msg::DEBUG << tr("Booking global memory block: %1").arg(gname)
            << msg::endmsg;
   return;
#endif  // HAVE_CERNLIB
   ...
   if (!m_initialized) {
      ...
      if (!name) {
         REPORT_ERROR(
             tr("Couldn't const_cast specified global "
                "memory name"));
         return;
      }
#ifdef HAVE_CERNLIB
 ...
 */
int HistMgr::book_1d(const char* name, int channels, double lowerBound,
                     double upperBound) {
 ...
 * @returns The ID with which the histogram was created
 */
int HistMgr::book_2d(const char* name, int xchannels, double xlow, double xup,
                     int ychannels, double ylow, double yup) {
   ...
   // Book the histogram:
#ifdef HAVE_CERNLIB
   HBOOK2(m_counter, lname, xchannels, xlow, xup, ychannels, ylow, yup, 0.);
#endif  // HAVE_CERNLIB
 ...
 * @param weight The weight of the event
 */
void HistMgr::fill_2d(int id, double xdata, double ydata, double weight) const {
