...
TextStream& TextStream::operator<<(const Message& message) {
   ...
   if (message.getSender().length() < MAXIMUM_SENDER_LENGTH) {
      sender =
          message.getSender().leftJustified(MAXIMUM_SENDER_LENGTH, ' ', true);
   } else {
   ...
   if ((level_it = m_levelMap.find(message.getLevel())) == m_levelMap.end()) {
      // This is a bad level!
   ...
   // of the last line):
   //
   for (QStringList::const_iterator text = texts.begin(); text != texts.end();
        ++text) {

      (*(QTextStream*)this) << time << " " << sender << " " << level << " "
                            << *text << ((text + 1 == texts.end()) ? "" : "\n");
   }
