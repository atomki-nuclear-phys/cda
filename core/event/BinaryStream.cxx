...
BinaryStream::BinaryStream(QIODevice* device)
    : QDataStream(device), m_logger("ev::BinaryStream") {}

BinaryStream::BinaryStream(QByteArray* array, QIODevice::OpenMode openMode)
    : QDataStream(array, openMode), m_logger("ev::BinaryStream") {}
...
BinaryStream& BinaryStream::operator>>(Event& event) {
   ...
   if (!ensureDataAvailable(4)) {
      m_logger << msg::WARNING << tr("Incomplete event received")
               << msg::endmsg;
...
BinaryStream& BinaryStream::operator>>(Fragment& fragment) {
   ...
   if (!ensureDataAvailable(sizeof(moduleID) + sizeof(nDataWords))) {
      m_logger << msg::WARNING << tr("Incomplete event received")
               << msg::endmsg;
      throw IncompleteEvent();
      return *this;
...
bool BinaryStream::ensureDataAvailable(qint64 minBytes) {
   ...
   if (device()->bytesAvailable() >= minBytes)
      return true;
...
bool BinaryStream::ensureDataSent(qint64 maxBytes) {
   ...
   // Check if the requested amount of data is available:
   if (device()->bytesToWrite() < maxBytes)
      return true;
