...
TextStream::TextStream(FILE* fileHandle) : QTextStream(fileHandle) {}

TextStream::TextStream(QString* string) : QTextStream(string) {}

TextStream::TextStream(QByteArray* array) : QTextStream(array) {}
...
TextStream& TextStream::operator<<(const Event& event) {
   ...
   if (event.getFragments().size()) {
      ...
      (*(QTextStream*)this)
          << qApp->translate("ev::TextStream", " No event fragments!\n");
   }
...
TextStream& TextStream::operator<<(const Fragment& fragment) {
   ...
   if (fragment.getDataWords().size()) {
      ...
      std::vector<uint32_t>::const_iterator end = fragment.getDataWords().end();
      for (; itr != end; ++itr) {
         (*(QTextStream*)this)
             << qApp->translate("ev::TextStream", "   Data word: 0x%1\n")
                    .arg(QString::number(*itr, 16));
      }
   } else {
      (*(QTextStream*)this)
          << qApp->translate("ev::TextStream", "   No data words!\n");
   }
