...
uint32_t decodeUnderThreshold(uint32_t data, caen::VmeDevice::DeviceType type) {
   switch (type) {
      case caen::VmeDevice::DEV_V775A:
...
public:
   ...
   /// Destructor
   ~VmeDevicePrivateData() { delete[] m_buffer; }
...
#ifdef HAVE_CAEN_QTP_LIBS
   CHECK(cvt_V792_open(m_data->data(), address, bus.handle(), ::convert(type)));
#endif  // HAVE_CAEN_QTP_LIBS
...
#endif  // HAVE_CAEN_QTP_LIBS
   REPORT_VERBOSE(tr("Device type \"%1\" closed").arg(toString(m_type)));
...
/// @returns The usual status codes
///
StatusCode VmeDevice::setZeroSuppression(
    bool enable, bool stepThreshold, const std::vector<uint16_t>& thresholds) {
   ...
   // correctly there...
#ifndef WIN32
   const size_t requiredSize = CVT_QTP_NUM_CHANNELS[m_data->data()->m_type];
   CHECK(thresholds.size() == requiredSize);
#endif  // WIN32
   // Call the setter function:
   CHECK(cvt_V792_set_zero_suppression(m_data->data(), enable, stepThreshold,
                                       thresholds.data()));
#endif  // HAVE_CAEN_QTP_LIBS
...
/// @returns The usual status codes
///
StatusCode VmeDevice::setAcquisitionMode(
    bool slidingScaleEnable, bool zeroSuppressionEnable,
    bool overflowSuppressionEnable, bool validSuppressionEnable,
    bool commonStopEnable, bool emptyEnable, bool countAllEvents) {
...
#ifdef HAVE_CAEN_QTP_LIBS
   // Call the setter function:
   CHECK(cvt_V792_set_acquisition_mode(
       m_data->data(), slidingScaleEnable, zeroSuppressionEnable,
       overflowSuppressionEnable, validSuppressionEnable, commonStopEnable,
       emptyEnable, countAllEvents));
#endif  // HAVE_CAEN_QTP_LIBS
   ...
   m_logger << msg::DEBUG << tr("Set acquisition mode with:\n")
            ...
            << "countAllEvents           : " << countAllEvents << msg::endmsg;
...
/// @returns The usual status codes
///
StatusCode VmeDevice::setReadoutMode(bool busErrorEnable, bool blockEndEnable,
                                     bool align64Enable) {
...
StatusCode VmeDevice::setPedestal(uint8_t value) {
   ...
   // Tell the user what happened:
   m_logger << msg::DEBUG << tr("Set the pedestal to: %1").arg(value, 2, 16)
            << msg::endmsg;
...
StatusCode VmeDevice::writeBitSet1Register(uint16_t value) {
   ...
   m_logger
       << msg::DEBUG
       << tr("Wrote value (bin) %1 to the bit set 1 register").arg(value, 16, 2)
       << msg::endmsg;
...
StatusCode VmeDevice::writeBitSet2Register(uint16_t value) {
   ...
   m_logger
       << msg::DEBUG
       << tr("Wrote value (bin) %1 to the bit set 2 register").arg(value, 16, 2)
       << msg::endmsg;
...
StatusCode VmeDevice::read(std::vector<DataEvent>& events) {
   ...
   while (m_data->bufferUsage() >= 4) {
      ...
      switch (data & CVT_QTP_DATA_TYPE_MSK) {
         ...
         case CVT_QTP_EOB:
            ...
            currentEvent.footer.eventCount = CVT_QTP_GET_EOB_EVENT_COUNT(data);
         ...
         case CVT_QTP_DATUM:
            ...
            currentEvent.data.back().overflow = ::decodeOverflow(data, m_type);
            break;
   ...
   if (m_data->bufferUsage() > 0) {
      REPORT_ERROR(
          tr("Data words were left in the MEB. This will lead "
             "to data loss."));
   }
#else
   ...
   for (int i = 0; i < nevents; ++i) {
      ...
      for (int j = 0; j < 32; ++j) {
         ...
         event.data.back().data = (j * 100 + 100 + (rand() / (RAND_MAX / 20)));
