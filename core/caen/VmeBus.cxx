...
VmeBus::VmeBus() : m_handle(-1), m_logger("caen::VME") {
...
StatusCode VmeBus::open(BoardType type, short linkNumber, short boardNumber) {
...
#ifdef HAVE_CAEN_VME_LIBS
   CHECK(CAENVME_Init(convert(type), linkNumber, boardNumber, &m_handle));
#endif  // CAEN libs
...
StatusCode VmeBus::close() {
   ...
   m_logger << msg::DEBUG << tr("Closed connection to VME device")
            << msg::endmsg;
...
#ifdef HAVE_CAEN_VME_LIBS
   CHECK(CAENVME_WriteCycle(m_handle, address, &value, cvA32_U_DATA, cvD8));
#endif  // CAEN libs
...
StatusCode VmeBus::write(unsigned int address, uint16_t value) const {
   ...
   // Perform a standard write with 32-bit addressing.
#ifdef HAVE_CAEN_VME_LIBS
   CHECK(CAENVME_WriteCycle(m_handle, address, &value, cvA32_U_DATA, cvD16));
#endif  // CAEN libs
...
#ifdef HAVE_CAEN_VME_LIBS
   CHECK(CAENVME_ReadCycle(m_handle, address, &value, cvA32_U_DATA, cvD8));
#endif  // CAEN libs
...
StatusCode VmeBus::read(unsigned int address, uint16_t& value) const {
   ...
   // Perform a standard read with 32-bit addressing.
#ifdef HAVE_CAEN_VME_LIBS
   CHECK(CAENVME_ReadCycle(m_handle, address, &value, cvA32_U_DATA, cvD16));
#endif  // CAEN libs
