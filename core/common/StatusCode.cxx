...
public:
   ...
   ~UncheckedCounter() {
      ...
      if (m_uncheckedSuccess) {
         std::cerr << "Warning in <StatusCode>: "
                   << "Number of unchecked successes: " << m_uncheckedSuccess
                   << std::endl;
      }
      if (m_uncheckedFailure) {
         std::cerr << "Error in   <StatusCode>: "
                   << "Number of unchecked failures: " << m_uncheckedFailure
                   << std::endl;
      }
...
///
/// By default applications will fail on an uncecked status code. In order to
/// disable this behaviour, the application has to turn it off explicitly.
///
static bool s_failure = false;

StatusCode::StatusCode(unsigned long rstat) : m_code(rstat), m_checked(false) {}
