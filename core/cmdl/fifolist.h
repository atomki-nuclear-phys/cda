...
//
//    09/01/98  Igor Soloviev   <Igor.Soloviev@cern.ch>
//    - Added right definition of name2 for MS VC++ 5.0
//-^^---------------------------------------------------------------------
...
# endif
   ...
   //
class CmdLineExport GenericFifoList {
private:
...
protected:
   ...
   // : head(0), tail(0), num_items(0), del_items(0), mod(0) {}
	...
	// @@ 04/02/99 Igor Soloviev
	// Change order to avoid compiler warnings
      : mod(0), del_items(0), num_items(0), head(0), tail(0) {}
...
class CmdLineExport  GenericFifoListIter {
private:
...
class CmdLineExport  GenericFifoListArray {
private:
