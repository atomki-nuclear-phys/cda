...
ConfServer::ConfServer(const QString& configFileName, QObject* parent)
   ...
   if (dev::Loader::instance()->loadAll()) {
      m_logger << msg::DEBUG << tr("Successfully loaded all available devices")
               << msg::endmsg;
   } else {
   ...
   //
   connect(this, SIGNAL(newConnection()), this, SLOT(sendConfiguration()));
}
...
void ConfServer::sendConfiguration() {
   ...
   REPORT_VERBOSE(
       tr("Bytes to be written: %1").arg(connection->bytesToWrite()));
...
bool ConfServer::readConfiguration() {
   ...
   m_logger
       << msg::DEBUG
       << tr("Size of configuration buffer: %1").arg(m_buffer.buffer().size())
       << msg::endmsg;
...
bool ConfServer::readBinaryConfig() {
   ...
   if (!crate.writeConfig(m_buffer)) {
      REPORT_ERROR(
          tr("Some error happened while writing the "
             "configuration\n"
      ...
      m_logger << msg::DEBUG
               << tr("Updated internal buffer from: %1").arg(m_configFileName)
               << msg::endmsg;
   }
...
bool ConfServer::readXMLConfig() {
   ...
   if (!doc.setContent(&configFile, false, &errorMsg, &errorLine,
      ...
      REPORT_VERBOSE(tr("Successfully parsed: %1").arg(m_configFileName));
   }
