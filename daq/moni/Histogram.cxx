...
const int Histogram::MIN_Y_TICK_DISTANCE = 80;

const int Histogram::X_AXIS_SPACING = 100;
const int Histogram::Y_AXIS_SPACING = 120;
...
Histogram::Histogram(QWidget* parent, Qt::WindowFlags flags)
   ...
   setMinimumSize(220. * devicePixelRatio(), 150. * devicePixelRatio());
...
Histogram::Histogram(const QString& title, int bins, double low, double up,
                     int refreshTimeout, QWidget* parent, Qt::WindowFlags flags)
    : QWidget(parent, flags),
      m_title(title),
      m_nbins(bins),
      m_low(low),
      ...
      m_xAxisStyle(Linear),
      m_yAxisStyle(Linear),
      m_values(static_cast<size_t>(bins + 2), 0.0),
      m_entries(0),
      m_logger("moni::Histogram") {
...
void Histogram::paintEvent(QPaintEvent*) {
   ...
   painter.drawRect(
       QRectF(0, 0, width() - pen.width(), height() - pen.width()));

   // Draw the rectangle of the histogram:
   painter.drawRect(
       QRectF(Y_AXIS_SPACING * devicePixelRatio(), 20. * devicePixelRatio(),
              width() - devicePixelRatio() * (Y_AXIS_SPACING + 20.),
              height() - devicePixelRatio() * (X_AXIS_SPACING + 20.)));
   ...
   if (width() > 300. * devicePixelRatio()) {
      painter.setPen(Qt::SolidLine);
      painter.drawText(QRectF(0, height() - 35. * devicePixelRatio(), width(),
                              25. * devicePixelRatio()),
...
void Histogram::mousePressEvent(QMouseEvent* event) {
   ...
   if (event->button() == Qt::LeftButton) {
      ...
      if ((event_y < (height() - X_AXIS_SPACING - 20)) ||
          ...
          (event_x < Y_AXIS_SPACING) || (event_x > width() - 20)) {
         return;
      }
   ...
   AxisStyleAction* xLinAction =
       new AxisStyleAction(Linear, tr("Linear"), xAxisMenu);
   connect(xLinAction, SIGNAL(triggered(Histogram::AxisStyle)), this,
           SLOT(setXAxisStyle(Histogram::AxisStyle)));
   // Only offer the logarithmic axis style if it makes sense:
   AxisStyleAction* xLogAction = 0;
   if (getLogXPossible()) {
      xLogAction =
          new AxisStyleAction(Logarithmic, tr("Logarithmic"), xAxisMenu);
      connect(xLogAction, SIGNAL(triggered(Histogram::AxisStyle)), this,
              SLOT(setXAxisStyle(Histogram::AxisStyle)));
   }
   ...
   AxisStyleAction* yLinAction =
       new AxisStyleAction(Linear, tr("Linear"), yAxisMenu);
   connect(yLinAction, SIGNAL(triggered(Histogram::AxisStyle)), this,
           SLOT(setYAxisStyle(Histogram::AxisStyle)));
   // Only offer the logarithmic axis style if it makes sense:
   AxisStyleAction* yLogAction = 0;
   if (getLogYPossible()) {
      yLogAction =
          new AxisStyleAction(Logarithmic, tr("Logarithmic"), yAxisMenu);
      connect(yLogAction, SIGNAL(triggered(Histogram::AxisStyle)), this,
              SLOT(setYAxisStyle(Histogram::AxisStyle)));
   }
   ...
   connect(unzoomAction, SIGNAL(triggered()), this, SLOT(unzoom()));
...
size_t Histogram::getBin(double value) const {
   ...
   if (value < m_low)
      return 0;
   if (value > m_up)
      return static_cast<size_t>(m_nbins + 1);

   return static_cast<size_t>(
       std::floor((value - m_low) / ((m_up - m_low) / m_nbins)) + 1);
}
...
void Histogram::drawXAxis(QPainter& painter) const {
   ...
   const qreal axis_start = Y_AXIS_SPACING * devicePixelRatio();
   ...
   for (; major_itr != major_end; ++major_itr) {
      ...
      if ((tick_location < axis_start) ||
          (tick_location > (width() - 20. * devicePixelRatio())))
         continue;

      // Draw the line of the tick:
      painter.setPen(Qt::SolidLine);
      painter.drawLine(
          QLineF(tick_location, height() - devicePixelRatio() * X_AXIS_SPACING,
                 tick_location,
      ...
      painter.drawText(
          QRectF(
              tick_location - devicePixelRatio() * (MIN_X_TICK_DISTANCE * 0.5),
              height() - devicePixelRatio() * (X_AXIS_SPACING - 20),
              devicePixelRatio() * MIN_X_TICK_DISTANCE,
              20. * devicePixelRatio()),
          Qt::AlignCenter, QString::number(tick_value, 'g', 4));

      // Draw the dotted line across the pad:
      painter.setPen(Qt::DotLine);
      painter.drawLine(QLineF(tick_location,
                              height() - devicePixelRatio() * X_AXIS_SPACING,
                              tick_location, 20. * devicePixelRatio()));
   }

   // If exponents were used, put that beside the axis:
   if (exp.first) {
      painter.drawText(
          QRectF(width() - 120. * devicePixelRatio(),
                 height() - 30. * devicePixelRatio(), 100. * devicePixelRatio(),
                 20. * devicePixelRatio()),
          Qt::AlignVCenter | Qt::AlignRight,
          // This should be painted much nicer once I'll have time:
          tr("x10^%1").arg(exp.second));
   }
...
void Histogram::drawYAxis(QPainter& painter) const {
   ...
   // Some basic value(s) about the axis:
   const int axis_start = height() - X_AXIS_SPACING * devicePixelRatio();
   ...
   for (; major_itr != major_end; ++major_itr) {
      ...
      const int tick_location =
          static_cast<int>(std::floor(axis_start - major_itr->position()));
      if ((tick_location < 20. * devicePixelRatio()) ||
      ...
      painter.drawLine(QLineF(
          (Y_AXIS_SPACING - TICK_LENGTH_MAJOR) * devicePixelRatio(),
          tick_location, Y_AXIS_SPACING * devicePixelRatio(), tick_location));
      ...
      // Now draw it on the axis:
      if (tick_location > 30. * devicePixelRatio()) {
         painter.drawText(QRectF(10. * devicePixelRatio(),
                                 tick_location - (MIN_Y_TICK_DISTANCE / 2.) *
                                                     devicePixelRatio(),
                                 (Y_AXIS_SPACING - TICK_LENGTH_MAJOR - 3.) *
                                     devicePixelRatio(),
                                 MIN_Y_TICK_DISTANCE * devicePixelRatio()),
                          Qt::AlignVCenter | Qt::AlignLeft,
                          QString::number(tick_value, 'g', 4));
      }

      // Draw the dotted line across the pad:
      painter.setPen(Qt::DotLine);
      painter.drawLine(QLineF(Y_AXIS_SPACING * devicePixelRatio(),
                              tick_location, width() - 20. * devicePixelRatio(),
                              tick_location));
   }
   ...
   if (exp.first) {
      painter.drawText(
          QRectF(5. * devicePixelRatio(), 5. * devicePixelRatio(),
                 100. * devicePixelRatio(), 20. * devicePixelRatio()),
          Qt::AlignVCenter | Qt::AlignLeft,
          // This should be painted much nicer once I'll have time:
          tr("x10^%1").arg(exp.second));
   }
   ...
   for (; minor_itr != minor_end; ++minor_itr) {
      ...
      painter.drawLine(QLineF(
          (Y_AXIS_SPACING - TICK_LENGTH_MINOR) * devicePixelRatio(),
          tick_location, Y_AXIS_SPACING * devicePixelRatio(), tick_location));
   }
}
...
void Histogram::drawHist(QPainter& painter) const {
   ...
   //
   const int x_axis_start = Y_AXIS_SPACING * devicePixelRatio();
   const int y_axis_start = height() - X_AXIS_SPACING * devicePixelRatio();
      ...
      const int y_bin_pos =
          (ybin.getDrawPosition(m_values[ihelp]) > 0.0
               ? static_cast<int>(std::floor(
                     y_axis_start - ybin.getDrawPosition(m_values[ihelp])))
               : y_axis_start);
      const int x_bin_low_pos = static_cast<int>(
          std::floor(xbin.getDrawPosition(m_low + i * bin_width)));
      const int x_bin_up_pos = static_cast<int>(
          std::floor(xbin.getDrawPosition(m_low + (i + 1) * bin_width)));

      // A security check:
      if ((x_bin_low_pos < 0.0) || (x_bin_up_pos < 0.0))
         continue;

      if (prev_pos > 0) {
         painter.drawLine(QLineF(x_axis_start + x_bin_low_pos, prev_pos,
                                 x_axis_start + x_bin_low_pos, y_bin_pos));
      }
      prev_pos = y_bin_pos;
      painter.drawLine(QLineF(x_axis_start + x_bin_low_pos, y_bin_pos,
...
void Histogram::drawZoom(QPainter& painter) const {
   ...
   painter.drawLine(QLineF(Y_AXIS_SPACING * devicePixelRatio() + m_zoomStart,
                           20. * devicePixelRatio(),
                           Y_AXIS_SPACING * devicePixelRatio() + m_zoomStart,
                           height() - X_AXIS_SPACING * devicePixelRatio()));

   // Draw a line at the current location of the zoom:
   painter.setPen(QPen(QBrush(Qt::red), 1));
   painter.drawLine(QLineF(Y_AXIS_SPACING * devicePixelRatio() + m_zoomCurrent,
                           20. * devicePixelRatio(),
                           Y_AXIS_SPACING * devicePixelRatio() + m_zoomCurrent,
                           height() - X_AXIS_SPACING * devicePixelRatio()));
...
void Histogram::drawStat(QPainter& painter) const {
   ...
   painter.setPen(Qt::SolidLine);
   painter.setBrush(Qt::white);
   painter.drawRect(QRectF(width() - 300. * devicePixelRatio(),
                           20. * devicePixelRatio(), 280. * devicePixelRatio(),
                           120. * devicePixelRatio()));

   // Draw the statistics as simple text:
   painter.drawText(
       QRectF(width() - 290. * devicePixelRatio(), 20. * devicePixelRatio(),
              270. * devicePixelRatio(), 120. * devicePixelRatio()),
...
Histogram::AxisBinning Histogram::getLinearAxisBinning(double low, double up,
   ...
   //
   double tick_major_unit =
       std::pow(10.0, std::ceil(std::log10(up - low)) - 1.0);
   ...
   const double tick_major_offset =
       (std::fmod(-1.0 * low, tick_major_unit) * axis_unit);
   const double tick_minor_offset =
       (std::fmod(-1.0 * low, tick_minor_unit) * axis_unit);
   ...
   if (std::abs(low) < 0.001) {
      ...
      if (low < 0.0) {
         first_major_tick_value =
             (std::ceil(low / tick_major_unit) * tick_major_unit);
      } else {
         first_major_tick_value =
             (std::floor(low / tick_major_unit) * tick_major_unit);
      }
   }
   ...
   for (int i = 0; i < n_major_ticks; ++i) {
      ...
      if ((position < 0.0) || (position > alength))
         continue;
      result.addMajorTick(AxisBinning::MajorTick(
          position, first_major_tick_value + i * tick_major_unit));
   }
 ...
 * @returns A structure describing the proposed axis decoration
 */
Histogram::AxisBinning Histogram::getLogarithmicAxisBinning(double low,
                                                            double up,
                                                            double alength,
                                                            int tdist) const {
   ...
   // The value associated with the first major tick mark:
   //
   const double first_major_tick_value = std::ceil(
   ...
   int exponent = static_cast<int>(
       std::floor(first_major_tick_value - tick_major_exponent_unit + 0.5));
   ...
   while (value < up) {
      ...
      if ((mvalue > low) && (mvalue < up) &&
         ...
         result.addMajorTick(
             AxisBinning::MajorTick(result.getDrawPosition(mvalue), mvalue));
      }
      for (int i = 1; i < 10; ++i) {
         ...
         if (value < low)
            continue;
         result.addMinorTick(
             AxisBinning::MinorTick(result.getDrawPosition(value)));
      }
      ++exponent;
   }
...
std::pair<double, double> Histogram::getYAxisLimits() const {
   ...
   const double maximum = *(
       std::max_element((m_values.begin() + start), (m_values.begin() + end)));
   double view_maximum = 0.0;
   const double minimum = *(
       std::min_element((m_values.begin() + start), (m_values.begin() + end)));
...
bool Histogram::getLogYPossible() const {
   ...
   const double minimum = *(std::min_element(m_values.begin(), m_values.end()));
...
Histogram::AxisBinning Histogram::getXAxisBinning() const {
   ...
   if (m_xAxisStyle == Linear) {
      ...
      return getLogarithmicAxisBinning(
          m_low, m_up, axis_length, devicePixelRatio() * MIN_X_TICK_DISTANCE);
   }
...
Histogram::AxisBinning Histogram::getYAxisBinning() const {
   ...
   if (m_yAxisStyle == Linear) {
      return getLinearAxisBinning(limits.first, limits.second, axis_length,
                                  devicePixelRatio() * MIN_Y_TICK_DISTANCE);
   } else if (m_yAxisStyle == Logarithmic) {
      return getLogarithmicAxisBinning(
          limits.first, limits.second, axis_length,
          devicePixelRatio() * MIN_Y_TICK_DISTANCE);
   }
...
std::pair<bool, int> Histogram::getExponent(
    const Histogram::AxisBinning& abin) const {
   ...
   if (abin.getStyle() == Linear) {
      if ((abin.majors().front().value() < -1000.0) ||
         ...
         const int exponent = static_cast<int>(std::floor(
             std::log10(std::max(std::abs(abin.majors().front().value()),
      ...
      if (abin.majors().back().value() > 1000.0) {
         ...
         std::vector<AxisBinning::MajorTick>::const_iterator end =
             abin.majors().end();
         for (; itr != end; ++itr) {
            exponent += static_cast<int>(std::floor(std::log10(itr->value())));
         }
...
Histogram::AxisBinning::tick_value Histogram::AxisBinning::MajorTick::value()
    const {
...
void Histogram::AxisBinning::addMajorTick(
    const Histogram::AxisBinning::MajorTick& tick) {

   m_majors.push_back(tick);
   return;
}

void Histogram::AxisBinning::addMinorTick(
    const Histogram::AxisBinning::MinorTick& tick) {
