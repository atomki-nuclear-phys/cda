...
public:
   ...
   RateWindow(QWidget* parent = 0, Qt::WindowFlags flags = Qt::WindowFlags());
   /// Destructor
   ~RateWindow();
...
private:
   ...
   QLabel* m_processedEventsLabel;  ///< Description of the number of processed
                                    ///< events
   ...
   QTimer*
       m_rateTimer;  ///< Timer used to periodically update the displayed rate
   qreal m_currentRate;    ///< Last calculated value of the rate
   Statistics m_lastStat;  ///< Last statistics object received

   QTimer*
       m_resetTimer;  ///< Timer used to reset the shown rate after a timeout
