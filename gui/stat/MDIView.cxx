...
MDIView::MDIView(QWidget* parent, Qt::WindowFlags flags)
   ...
   // Create a simple table that displays a quick overview of the event
   // processing rates of the whole CDA DAQ system:
   ...
   m_rateTable->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);
   ...
   m_rateTable->setColumnWidth(
       0, m_rateTable->viewport()->width() - TABLE_RATE_COLUMN_WIDTH);
 ...
 * came from a source that is already known, then it just gives the object to
 * the window handling the information from that source. If it's a new source,
 * then it creates a new stat::RateWindow and a new row in the rate table for
 * it. Then it lets the sub-window do its job with the statistics object...
 */
void MDIView::handleStatistics(const Statistics& stat) {
   ...
   if (m_windowMap.contains(stat.getSource())) {
      ...
      subwin->setMinimumSize(window->width() + 20, window->height() + 30);
      subwin->setMaximumSize(window->width() + 20, window->height() + 30);

      // Connect the signals of this new sub-window:
      connect(window, SIGNAL(newRateAvailable(const QString&, qreal)), this,
              SLOT(updateRateTable(const QString&, qreal)));
      connect(window, SIGNAL(aboutToClose(const QString&)), this,
              SLOT(handleWindowClose(const QString&)));
      connect(subwin, SIGNAL(aboutToActivate()), window, SLOT(update()));
...
/**
 * This slot is responsible for periodically updating the event processing rates
...
/**
 * When a sub-window is closed, the associated table row is deleted from the
 * rate table by this function.
 *
 * @param source Source ID of the rate information
 */
...
/**
 * This simple function adjusts the two columns of the rate table so that they
