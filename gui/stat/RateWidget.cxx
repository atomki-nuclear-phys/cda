...
RateWidget::RateWidget(QWidget* parent, Qt::WindowFlags flags)
    : QWidget(parent, flags), m_rateValues(RATE_HISTORY_LENGTH, 0.0) {
...
void RateWidget::paintEvent(QPaintEvent*) {
   ...
   painter.drawText(QRect(0, 0, width(), 20), Qt::AlignCenter,
                    tr("Peak: %1 Hz").arg(QString::number(maximum)));
   ...
   const QPoint edge1[4] = {QPoint(0, 20), QPoint(0, height()),
                            QPoint(5, height() - 5), QPoint(5, 25)};
   const QPoint edge2[4] = {QPoint(0, 20), QPoint(5, 25),
                            QPoint(width() - 5, 25), QPoint(width(), 20)};
   painter.setPen(Qt::SolidLine);
   ...
   painter.drawLine(QLine(width() - 5, height() - 5, width(), height()));
   ...
   const qreal mark_width =
       static_cast<qreal>(width() - 10.0) / RATE_HISTORY_LENGTH;
   ...
   for (std::list<qreal>::const_iterator it = m_rateValues.begin();
      ...
      const int marks = static_cast<int>(
          std::floor(((height() - 30) / mark_height) * (*it / view_maximum)));
      for (int mark = 0; mark < marks; ++mark) {
         const qreal rect_x = 5 + column * mark_width;
         const qreal rect_y =
             (height() - 5 - mark_height) - (mark * mark_height);
         painter.drawRect(
             QRectF(rect_x, rect_y,
                    (column == RATE_HISTORY_LENGTH - 1 ? width() - 5 - rect_x
                                                       : mark_width),
                    mark_height));
      }
   }
