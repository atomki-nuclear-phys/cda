...
RateWindow::RateWindow(QWidget* parent, Qt::WindowFlags flags)
   ...
   connect(m_rateTimer, SIGNAL(timeout()), this, SLOT(updateRate()));
   m_rateTimer->start();

   m_resetTimer = new QTimer(this);
   m_resetTimer->setInterval(RATE_RESET_TIMEOUT);
   m_resetTimer->setSingleShot(true);
   connect(m_resetTimer, SIGNAL(timeout()), this, SLOT(resetStatistics()));
}
...
RateWindow::~RateWindow() {
   ...
   delete m_rateWidget;
   delete m_sourceNameLabel;
   delete m_sourceName;
   delete m_processedEventsLabel;
   delete m_processedEvents;
...
/**
 * This slot resets the event processing rate after no statistics information
 * has been received for RATE_RESET_TIMEOUT length of time.
 */
