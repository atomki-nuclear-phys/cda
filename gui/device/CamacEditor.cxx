...
CamacEditor::CamacEditor(QWidget* parent, Qt::WindowFlags flags)
   ...
   m_crateView->setGeometry(
       QRect(0, 0, CamacCrateWidget::WIDTH, CamacCrateWidget::HEIGHT));
   connect(m_crateView.get(), SIGNAL(doubleClicked(int)), this,
   ...
   m_deviceWidget->setGeometry(
       QRect(10, 10, CamacGui::WIDGET_WIDTH, CamacGui::WIDGET_HEIGHT));
   connect(m_deviceWidget.get(), SIGNAL(widgetRemoved(int)), this,
           SLOT(removeDeviceSlot(int)));
}
...
void CamacEditor::showDeviceSlot(int slot) {
   ...
   if (!device) {
      m_logger << msg::DEBUG << tr("There is no device in slot %1").arg(slot)
               << msg::endmsg;
      return;
   }
