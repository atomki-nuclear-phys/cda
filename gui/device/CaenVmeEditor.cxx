...
CaenVmeEditor::CaenVmeEditor(QWidget* parent, Qt::WindowFlags flags)
   ...
   for (QStringList::const_iterator dev = devices.begin(); dev != devices.end();
        ++dev) {
      ...
      m_createDevice->addItem(m_loader->getFactory(*dev).longName(), *dev);
   }

   // Connect the dropdown menu to the slot handling the creation of
   // a CAEN VME device:
   connect(m_createDevice.get(), SIGNAL(currentIndexChanged(int)), this,
           SLOT(createDeviceSlot(int)));

   // Create a widget to show the devices in:
   m_deviceTab.reset(new QTabWidget(this));
   m_deviceTab->setGeometry(QRect(0, 30, 520, CaenVmeGui::WIDGET_HEIGHT + 40));
   m_deviceTab->setUsesScrollButtons(true);
   m_deviceTab->setTabsClosable(true);
   connect(m_deviceTab.get(), SIGNAL(tabCloseRequested(int)), this,
           SLOT(deleteDeviceSlot(int)));
   ...
   for (; itr != end; ++itr) {
      ...
      m_deviceTab->addTab(itr->second.get(),
                          QString("%1 [%2]")
                              ...
                              .arg(itr->second->getID(), 4, 16, QChar('0')));
      // Connect its signal(s):
      connect(itr->second.get(), SIGNAL(addressChanged()), this,
              SLOT(addressChangedSlot()));
   }
...
void CaenVmeEditor::clear() {
   ...
   if (!consistent()) {
      REPORT_ERROR(
          tr("The GUI is not consistent after ID change "
             "handling"));
      return;
   }
...
StatusCode CaenVmeEditor::writeCrateConfig(QDomElement& node) const {
   ...
   node.setAttribute("ControllerType", static_cast<int>(m_vmeWidget->type()));
...
void CaenVmeEditor::createDeviceSlot(int index) {
   ...
   // Make sure the combo box is in the first position:
   m_createDevice->setEnabled(
       false);  // So that the function doesn't call itself
   m_createDevice->setCurrentIndex(0);
   m_createDevice->setEnabled(true);
   ...
   //
   std::unique_ptr<CaenVmeGui> device = factory.createDevice<CaenVmeGui>();
   if (!device.get()) {
   ...
   // Connect its signal(s):
   connect(device.get(), SIGNAL(addressChanged()), this,
           SLOT(addressChangedSlot()));
...
void CaenVmeEditor::deleteDeviceSlot(int index) {
   ...
   CaenVmeGui* device = dynamic_cast<CaenVmeGui*>(m_deviceTab->widget(index));
   if (!device) {
...
bool CaenVmeEditor::consistent() const {
   ...
   return (static_cast<int>(m_devices.size() + 1) == m_deviceTab->count());
}
