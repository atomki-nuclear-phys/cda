...
CaenDigitizerEditor::CaenDigitizerEditor(QWidget* parent, Qt::WindowFlags flags)
   ...
   m_deviceTab->setGeometry(
       QRect(0, 30, 520, CaenDigitizerGui::WIDGET_HEIGHT + 40));
...
StatusCode CaenDigitizerEditor::readConfig(QIODevice& dev) {
   ...
   for (; itr != end; ++itr) {
      ...
      connect(itr->second.get(), SIGNAL(idChanged()), this,
              SLOT(idChangedSlot()));
   }
...
void CaenDigitizerEditor::createDeviceSlot(int index) {
   ...
   // Connect its signal(s):
   connect(device.get(), SIGNAL(idChanged()), this, SLOT(idChangedSlot()));
