...
const int CamacCrateWidget::HEIGHT =
    CamacCrateWidget::SLOT_HEIGHT + 2 * CamacCrateWidget::BORDER_SIZE + 30;
...
StatusCode CamacCrateWidget::readConfig(QIODevice& dev) {
   ...
   for (; dev_itr != dev_end; ++dev_itr) {
      connect(dev_itr->second.get(), SIGNAL(redrawModule()), this,
              SLOT(update()));
   }
   //
   // Redraw the widget:
 ...
 */
void CamacCrateWidget::setDevice(int slot, std::unique_ptr<CamacGui> device) {
   ...
   DeviceMap_t::iterator dev = m_devices.find(slot);
   if (dev != m_devices.end()) {
      m_logger << msg::WARNING << tr("Redefining device in slot %1").arg(slot)
               << msg::endmsg;
   }
   ...
   //
   connect(ptr, SIGNAL(redrawModule()), this, SLOT(update()));
   update();
...
void CamacCrateWidget::paintEvent(QPaintEvent* /* event */) {
   ...
   painter.drawRect(QRect(0, 0, WIDTH, BORDER_SIZE));
   painter.drawRect(
       QRect(0, SLOT_HEIGHT + BORDER_SIZE, WIDTH, BORDER_SIZE + 30));
   painter.drawRect(QRect(0, 0, BORDER_SIZE, HEIGHT));
   painter.drawRect(QRect(WIDTH - BORDER_SIZE, 0, BORDER_SIZE, HEIGHT));

   painter.setPen(Qt::SolidLine);
   painter.setBrush(Qt::NoBrush);
   painter.drawRect(
       QRect(BORDER_SIZE, BORDER_SIZE, WIDTH - 2 * BORDER_SIZE, SLOT_HEIGHT));
   ...
   for (int i = 0; i < NUMBER_OF_SLOTS; ++i) {
      painter.setBrush(Qt::NoBrush);
      painter.drawRect(
          QRect(BORDER_SIZE + i * SLOT_WIDTH, BORDER_SIZE + SLOT_HEIGHT,
                (i == NUMBER_OF_SLOTS - 1 ? 2 * SLOT_WIDTH : SLOT_WIDTH),
                SLOT_WIDTH + 5));
      painter.setBrush(Qt::black);
      painter.drawText(
          QRect(BORDER_SIZE + i * SLOT_WIDTH, BORDER_SIZE + SLOT_HEIGHT + 5,
   ...
   painter.drawRect(QRect(BORDER_SIZE + 23 * SLOT_WIDTH, BORDER_SIZE,
                          2 * SLOT_WIDTH, SLOT_HEIGHT));
   painter.translate(BORDER_SIZE + 23 * SLOT_WIDTH, BORDER_SIZE);
   ...
   painter.drawText(QRect(0, 0, 2 * SLOT_WIDTH, 20), Qt::AlignCenter, "CC32");
   painter.translate(-(BORDER_SIZE + 23 * SLOT_WIDTH), -BORDER_SIZE);
   ...
   for (; dev_itr != dev_end; ++dev_itr) {
      ...
      painter.translate(
          BORDER_SIZE + (static_cast<int>(dev_itr->first) - 1) * SLOT_WIDTH,
          BORDER_SIZE);
   ...
   for (int i = 0; i < NUMBER_OF_SLOTS - 1; ++i) {
      painter.drawLine(QLine(BORDER_SIZE + SLOT_WIDTH + i * SLOT_WIDTH,
                             BORDER_SIZE,
                             BORDER_SIZE + SLOT_WIDTH + i * SLOT_WIDTH,
   ...
   if (event->button() == Qt::LeftButton) {
      ...
      if (m_loader) {
         ...
         for (QStringList::const_iterator dev = devices.begin();
            ...
            connect(ac, SIGNAL(triggered(int, const QString&)), this,
                    SLOT(createSlot(int, const QString&)));
      ...
      if (getDevice(slot)) {
         ...
         connect(ac, SIGNAL(triggered(int)), this, SLOT(clearSlot(int)));
         menu->addAction(ac);
      }
