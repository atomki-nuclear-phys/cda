...
RawWriterRunner::RawWriterRunner(QWidget* parent, Qt::WindowFlags flags)
    : QWidget(parent, flags), m_runner(), m_logger("sd::RawWriterRunner") {
   ...
   m_runner.setExecName(daq::PathResolver::resolve("cda-raw-writer", "PATH"));
   if (m_runner.getExecName() == "") {
      m_logger << msg::WARNING << tr("RAW file writing not available")
               << msg::endmsg;
...
void RawWriterRunner::startApp(bool start) {
   ...
   if (start) {
      ...
      if (!m_runner.start()) {
         ...
         m_logger << msg::INFO << tr("Raw writer started") << msg::endmsg;
      ...
      if (!m_runner.stop()) {
         REPORT_ERROR(
             tr("The Raw writer could not be stopped "
                "successfully"));
