...
namespace simple_daq {
   ...
   RootWriterRunner::RootWriterRunner( QWidget* parent, Qt::WindowFlags flags )
      ...
      m_runner.setExecName( daq::PathResolver::resolve( "cda-root-writer",
                                                        "PATH" ) );
    ...
    */
   void RootWriterRunner::setStatServerAddress( bool status, const QString& address ) {
   ...
   void RootWriterRunner::startApp( bool start ) {
      ...
      if( start ) {
         ...
         if( ! m_runner.start() ) {
            ...
            m_logger << msg::INFO << tr( "Root writer started" )
                     << msg::endmsg;
         ...
         if( ! m_runner.stop() ) {
            REPORT_ERROR( tr( "The Root writer could not be stopped "
                              "successfully" ) );
