...
struct CaenVmeReaderRunner::Impl {
...
CaenVmeReaderRunner::CaenVmeReaderRunner(QWidget* parent, Qt::WindowFlags flags)
    : QWidget(parent, flags),
      m_impl{new Impl{*this, tr("VME acquisition control"), tr("Start time:"),
                      tr("N/A"), tr("Stop time:"), tr("N/A"),
                      tr("Start VME reader"), tr("VME reader stopped")}} {
   ...
   m_impl->m_runner.setExecName(
       daq::PathResolver::resolve("cda-caen-vme-reader", "PATH"));
   if (m_impl->m_runner.getExecName() == "") {
      QMessageBox::critical(this, tr("Error"),
                            tr("cda-caen-vme-reader not found. Data "
                               "acquisition is not possible."));
      m_impl->m_mainBox.setEnabled(false);
   }
}

CaenVmeReaderRunner::~CaenVmeReaderRunner() {}
...
void CaenVmeReaderRunner::startApp(bool start) {
   ...
   if (start) {
      ...
      if (!m_impl->m_runner.start()) {
         ...
         //
         m_impl->m_processStatus.setText(tr("VME reader running"));
         QPalette palette(m_impl->m_processStatus.palette());
         palette.setColor(QPalette::Active, QPalette::WindowText,
                          QColor(150, 10, 10));
         palette.setColor(QPalette::Inactive, QPalette::WindowText,
         ...
         m_impl->m_starterButton.setText(tr("Stop VME reader"));
      ...
      if (!m_impl->m_runner.stop()) {
         ...
         //
         m_impl->m_processStatus.setText(tr("VME reader stopped"));
         QPalette palette(m_impl->m_processStatus.palette());
         palette.setColor(QPalette::Active, QPalette::WindowText,
                          QColor(10, 150, 10));
         palette.setColor(QPalette::Inactive, QPalette::WindowText,
      ...
      m_impl->m_starterButton.setText(tr("Start VME reader"));
